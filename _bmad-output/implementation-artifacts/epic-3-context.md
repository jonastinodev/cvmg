...
- Story 3.2 a introduit `constantes-express.php` (constante `PRIX_DEBLOCAGE_AR`, actuellement 10 000). Story 3.3 doit inclure ce même fichier pour son affichage du prix plutôt que de dupliquer le nombre — c'est tout l'intérêt de l'avoir extrait.
- Story 3.2 a introduit la table `deblocages` (`cv_id`, `operateur_id`, `horodatage`, `prix_ar`) et l'endpoint `debloquer-contact.php`, réservé aux sessions opérateur (`$_SESSION['est_operateur']`), qui revalide les mêmes conditions de visibilité que `recherche.php` avant de révéler un numéro. Aucune identité employeur n'y transite (choix acté). Épic 5 (FR-14) pourra lire cette table directement pour ses métriques plutôt que construire son propre journal de déblocages.
